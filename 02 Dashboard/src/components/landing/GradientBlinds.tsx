    ...
    const loop = (t: number) => {
      ...
      if (mouseRef) {
        if (mouseRef.current) {
          const rect = container.getBoundingClientRect();
          const scale = (renderer as unknown as { dpr?: number }).dpr || 1;
          const x = (mouseRef.current.x - rect.left) * scale;
          const y = (rect.height - (mouseRef.current.y - rect.top)) * scale;
          if (x >= -rect.width && x <= rect.width * 2 && y >= -rect.height && y <= rect.height * 2) {
            mouseTargetRef.current = [x, y];
          }
        } else {
          // Pointer left — move to center to go uniform/dim
